---
title: "ML Feature Selection and Classifier Testing"
output: html_notebook
---
This notebook contains the code used to facilitate feature selection during optimization of the machine learning pipeline. It plots the features, correlation plots of the features, and calculates Fisher scores. It also compares multiple machine learning classifiers using caretEnsemble.

The first few chunks of this notebook after the Functions chunk show how the input data from Matlab and CellProfiler was combined, cleaned, and scaled. Rather than including all input files, the cleaned data has been saved and included in this repository, and can be loaded by the user in the following chunk and used to run the rest of the notebook. The code used to generate the cleaned data has been commented out so it does not produce an error. There may be small rounding errors compared to when this code was run with the raw input files. 

Additionally the classifier models which were generated are included in the repository as .rds files. Rerunning the classifier training does not produce identical classifiers, although the accuracy was similar when I tried it. The original classifier can be loaded and used to generate the dotplot at the end of the notebook.
...
mlListWrapper <- function(data, trainObjects, listOfMethods, trainControlFcn, features){
  readyToLearn <- data %>% 
    dplyr::select(-Experiment, -Normalization, -Coverslip, -Replicate, -Condition, -Image, -contains("Confidence"), -contains("econd")) %>%
...
```{r}
lvl = c("OneEnd", "BothEnds", "None")
metricsClean.V1 <- read.csv(paste(dirPath, "Analysis/Paper/InputForGitHub/metricsClean.csv", sep = ""))
...
```{r}
# Create training set with about equal number of cells coming from each experiment (Figure 1 and Figure 4). Began by training on control condition only. In later analyses, I decided it was better to train across all conditions.
metrics1Labeled <- merge(metricsClean.V1 %>% filter(str_detect(Experiment, "20211203")), dplyr::select(manualComb, Object, ManualScoreMcam)) %>%
  filter(Normalization == "Control")
metrics4Labeled <- merge(metricsClean.V1 %>% filter(str_detect(Experiment, "20211127")), dplyr::select(manualComb, Object, ManualScoreMcam)) %>%
  filter(Normalization == "Control")
set.seed(123)
trainIdx1 <- createDataPartition(metrics1Labeled$ManualScoreMcam, p = 0.5, list = F, times = 1)
set.seed(123)
trainIdx4 <- createDataPartition(metrics4Labeled$ManualScoreMcam, p = 0.8, list = F, times = 1)
trainObjects1 <- metrics1Labeled$Object[trainIdx1]
trainObjects4 <- metrics4Labeled$Object[trainIdx4]

trainObjects <- c(trainObjects1, trainObjects4)

# Add labels from manual scoring to the feature data to facilitate training and testing of classifiers.
metricsScaledV1.MCAM.Labeled <- metricsScaledV1.MCAM %>%
  merge(dplyr::select(manualComb, Object, ManualScoreMcam, ConfidenceMcam), by = "Object") # Length is 3,169 because cells in manualComb which had too small an area are removed during metrics cleaning

# Classifiers to try:
methodList <- c("gbm", "nb", "xgbTree", "LogitBoost", "extraTrees", "avNNet", "mlpWeightDecayML")

# Get list of features after feature selection based on manual evaluation to remove highly correlated variables, noisy variables, and variables which varied between experiments or coverslips:
# Get variable names for all features (no feature selection)

# Note that "allFeatures" is not all of the features in the scaled metrics; features that were measured for the second brightest object were omitted for testing here because they contain missing values. Some earlier optimization indicated that for ML models which can handle missing values, including these features did not noticeably improve accuracy. The code in the next line which was used to remove the features for the second brightest object inadvertently removed the "Texture_AngularSecondMoment" features from CellProfiler so these were not included during the classifier tuning shown in this notebook.
allFeatures <- metricsScaledV1.MCAM.Labeled %>%
  dplyr::select(-Experiment, -Normalization, -Coverslip, -Replicate, -Condition, -Image, -contains("econd"), -ManualScoreMcam, -ConfidenceMcam, -Object) %>%
  colnames()

reducedFeatures <- metricsScaledV1.MCAM.Labeled %>%
  dplyr::select(all_of(c(colnames(dplyr::select(metricsScaledV1.MCAM.Labeled, -ManualScoreMcam, -Object, -Experiment, -Normalization, -Coverslip, -Replicate, -Condition, -Image, -ConfidenceMcam))))) %>%
  dplyr::select(-contains("econd")) %>%
  dplyr::select(-contains("NotBright")) %>% 
  dplyr::select(-contains("minDist")) %>%
  dplyr::select(-contains("maxDist")) %>%
  dplyr::select(-contains("Eccentricity")) %>%
  dplyr::select(-contains("distCentrs")) %>% # highly correlated with other metrics
  dplyr::select(-contains("thirdQuartileDistCentr2")) %>%
  dplyr::select(-contains("DistWrampEdge")) %>% # lots of zeros, varies between experiments
  dplyr::select(-contains("Radius"), -AreaShape_EquivalentDiameter, -AreaShape_Perimeter) %>%
  dplyr::select(-contains("relAreaWrampCell")) %>% # Completely Redundant
  dplyr::select(-contains("areaBrightObj")) %>% # Completely Redundant
  dplyr::select(-contains("angle")) %>% # Not discriminating and confusing to calculate/interpret
  #dplyr::select(-contains("profile")) %>% # Not close to normal distribution
  dplyr::select(-contains("profileSecond")) %>%
  dplyr::select(-contains("numCandidates")) %>% # Discrete
  dplyr::select(-contains("medianObjectSumIntensity"), -contains("sumOverMedianIntensityWramp")) %>%
  dplyr::select(-contains("meanIntensityCell"), -contains("medianIntensityCell"), -contains("sumIntensityCell")) %>%
  dplyr::select(-contains("Texture_SumVariance_"), -contains("Texture_Contrast"), -contains("Texture_SumAverage"), -contains("Texture_Variance"), -contains("_Entropy"), -contains("SumEntropy")) %>% # highly correlated or very small range
  dplyr::select(-contains("2_Mean"), -contains("2_Range"), -contains("_256")) %>%
  dplyr::select(-contains("Zernike")) %>% # Depends on signal strength (not normalized, varies between experiments)
  dplyr::select(-contains("RadialDistribution_FracAtD_1"), -contains("RadialDistribution_FracAtD_2"), -contains("RadialDistribution_FracAtD_3")) %>% # Highly correlated with "FracAtD_4of4"
  dplyr::select(-contains("RadialDistribution_RadialCV")) %>% # Varies between experiments
  dplyr::select(-contains("Intensity_")) %>%
  colnames()
# write.csv(allFeatures, paste(dirPath, "Analysis/MachineLearning/MachineLearningDevelopment/MachineLearningWithVersionControl/MediansAndFeaturesForML/AllFeaturesMcam.csv", sep = ""))
# write.csv(reducedFeatures, paste(dirPath, "Analysis/MachineLearning/MachineLearningDevelopment/MachineLearningWithVersionControl/MediansAndFeaturesForML/SelectedFeaturesMcam.csv", sep = ""))
```

Machine learning with Caret Ensemble:
```{r}
# For 3 classes (OneEnd, BothEnds, or None)
set.seed(123)
myControl <- trainControl(method = "repeatedcv", 
                          ...
                          classProbs = T,
                          index = createMultiFolds(trainObjects, k = 10, times = 3))

# # Train classifiers using three-level classification
# set.seed(123)
# modelListV1 <-  mlListWrapper(metricsScaledV1.MCAM.Labeled, trainObjects, methodList, myControl, allFeatures)
# 
# set.seed(123)
# modelListV1.Reduced <- mlListWrapper(metricsScaledV1.MCAM.Labeled, trainObjects, methodList, myControl, reducedFeatures)
# 
# saveRDS(modelListV1, paste(dirPath, "Analysis/MachineLearning/MachineLearningDevelopment/MachineLearningWithVersionControl/Thesis/DevelopmentOfML/20220330ClassifiersForMcam/modelListV1.rds", sep = ""))
# saveRDS(modelListV1.Reduced, paste(dirPath, "Analysis/MachineLearning/MachineLearningDevelopment/MachineLearningWithVersionControl/Thesis/DevelopmentOfML/20220330ClassifiersForMcam/modelListV1Reduced.rds",sep = ""))
```

Load in models which were saved after running the code in the previous chunk which has been commented out. Note extraTrees can't be used to classify cells because it wasn't formatted for saving.
```{r}
modelListV1 <- readRDS(paste(dirPath, "Analysis/Paper/InputForGitHub/modelListV1.rds", sep = ""))
modelListV1.Reduced <- readRDS(paste(dirPath, "Analysis/Paper/InputForGitHub/modelListV1Reduced.rds",sep = ""))
```

Comparison of accuracy with feature selection or with all features except those for the second brightest object or TextureAngularSecond moment:
```{r}
analyzeResults(metricsScaledV1.MCAM.Labeled  %>%
                  dplyr::mutate(all = "All") %>% 
                 filter(!Object %in% trainObjects), modelListV1[["gbm"]], "ManualScoreMcam", "all")
# Reduced sensitivity without feature selection. Note that accuracy was not that different with or without feature selection for this dataset, but later datasets required feature selection for accurate training, possibly due to larger technical variation between experiments.

analyzeResults(metricsScaledV1.MCAM.Labeled  %>%
                  dplyr::mutate(all = "All") %>% 
                 filter(!Object %in% trainObjects), modelListV1.Reduced[["gbm"]], "ManualScoreMcam", "all")
```

Dotplots to compare the accuracy of the different fitted models:
```{r}
svglite(paste(dirPath, "Analysis/Paper/Output/Dotplot_3Class.svg", sep = ""))
dotplot(resamples(modelListV1), xlim=c(0,1))
dev.off

svglite(paste(dirPath, "Analysis/Paper/Output/Dotplot_3Class_Reduced.svg", sep = ""))
dotplot(resamples(modelListV1.Reduced), xlim=c(0,1))
dev.off()
```
