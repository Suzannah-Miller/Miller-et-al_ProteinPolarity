---
title: "Upsample Results for Thesis"
output: html_notebook
---

This notebook provides the code for testing the custom upSample function (where hard-to-classify cells are upsampled in successive classifier trainings). It uses the same data as "DevelopmentOfML_GitHub.Rmd." Thus, the first portions of this notebook are largely the same. 

As with the "DevelopmentOfML_GitHub.Rmd" notebook, to simplify rerunning of the code, the chunks of code used to compile and clean data from Matlab and CellProfiler has been commented out, and the cleaned data which is in the GitHub repository can be loaded in the next chunk. 

Additionally the classifier models which were generated are included in the repository as an .rds file. Rerunning the classifier training does not produce identical classifiers, although the accuracy was similar when I tried it. The original classifier can be loaded and used to reproduce the accuracy reported in the paper and the classified data reported in the supplemental data.
