---
title: "Development of Iterative Classifier Training"
output: html_notebook
---

This notebook provides the code for testing the custom upSample function (where hard-to-classify cells are upsampled in successive classifier trainings). 
It uses the same data as "DevelopmentOfML_GitHub.Rmd." Thus, the first portions of this notebook are largely the same. 

As with the "DevelopmentOfML_GitHub.Rmd" notebook, to simplify rerunning of the code, the chunks of code used to compile and clean data from Matlab and CellProfiler 
has been commented out, and the cleaned data which is in the GitHub repository can be loaded in the next chunk. 

Additionally the classifier models which were generated are included in the repository as an .rds file. Rerunning the classifier training does not produce identical classifiers, 
although the accuracy was similar when I tried it. The original classifier can be loaded and used to reproduce the accuracy reported in the paper and the classified data 
reported in the supplemental data.

Most of the code to save output externally has been commented out, except for the Supplemental Data file included in the paper which can be compared to the .csv file in the 
"OutputForGitHub" folder on GitHub.
...
# Wrapper function to run caretList on supplied training set and features
mlListWrapper <- function(data, trainObjects, listOfMethods, trainControlFcn, features){
  readyToLearn <- data %>% 
    dplyr::select(-Experiment, -Normalization, -Coverslip, -Replicate, -Condition, -Image, -Confidence, -contains("econd")) %>%
    dplyr::filter(Object %in% trainObjects)
  fitList <- caretList(readyToLearn %>% dplyr::select(any_of(features)),
            readyToLearn$ManualScoreMcam, 
            trControl = trainControlFcn,
...
plots2scale <- function(featureNames, data1, data2, chName, savePath, logOption){
  features <- str_replace(featureNames, "MCAM", chName)
  for(val in 1:length(features)){
    if(any(colnames(data1) == features[val])){
      p1 <- ggplot(data1 %>% rename(feature = features[val]), aes(x = Coverslip, y = feature, color = Experiment)) + 
    ...
    if(any(colnames(data2) == features[val])){
      p2 <- ggplot(data2 %>% rename(feature = features[val]), aes(x = Coverslip, y = feature, color = Experiment)) + 
      geom_boxplot(outlier.size = .5) +
      geom_jitter(height = 0, alpha = 0.4, size = 0.5) +
      ggtitle(features[val])+
      theme_classic() +
    ...
    ggsave(paste(savePath, features[val], ".png", sep = ""), p, width = 20, height = 6)
    
    if(logOption == T){
      if(any(colnames(data1) == features[val])){
        p1 <- ggplot(data1 %>% rename(feature = features[val]), aes(x = Coverslip, y = log(feature), color = Experiment)) + 
          geom_boxplot(outlier.size = .5) +
          geom_jitter(height = 0, alpha = 0.4, size = 0.5) +
          ggtitle(features[val])+
          theme_classic() +
      ...
      if(any(colnames(data2) == features[val])){
        p2 <- ggplot(data2 %>% rename(feature = features[val]), aes(x = Coverslip, y = log(feature), color = Experiment)) + 
        geom_boxplot(outlier.size = .5) +
        geom_jitter(height = 0, alpha = 0.4, size = 0.5) +
        ggtitle(features[val])+
        theme_classic() +
      ...
      p <- grid.arrange(p1, p2, nrow = 1)
      ggsave(paste(savePath, features[val], "_log.png", sep = ""), p, width = 20, height = 6)
    }
  }
}

plots2scaleSvg <- function(featureNames, data1, data2, chName, savePath, logOption){
  features <- str_replace(featureNames, "MCAM", chName)
  for(val in 1:length(features)){
    if(any(colnames(data1) == features[val])){
      p1 <- ggplot(data1 %>% rename(feature = features[val]), aes(x = Coverslip, y = feature, color = Experiment)) + 
    ...
    p <- grid.arrange(p1, p2, nrow = 1)
    ggsave(paste(savePath, features[val], ".svg", sep = ""), p, width = 20, height = 6)
    ...
    if(logOption == T){
      ...
      p <- grid.arrange(p1, p2, nrow = 1)
      ggsave(paste(savePath, features[val], "_log.svg", sep = ""), p, width = 20, height = 6)
    }
  }
}

plots1scale <- function(featureNames, data, chName, savePath, logOption){
  features <- str_replace(featureNames, "MCAM", chName)
  for(val in 1:length(features)){
  p <- ggplot(data %>% rename(feature = features[val]), aes(x = Coverslip, y = feature, color = Experiment)) + 
    geom_boxplot(outlier.size = .5) +
    geom_jitter(height = 0, alpha = 0.4, size = 0.5) +
    ggtitle(features[val])+
    theme_classic() +
  ...
  ggsave(paste(savePath, features[val], ".png", sep = ""), p, width = 10, height = 6)
  
  if(logOption == T){
      p <- ggplot(data %>% rename(feature = features[val]), aes(x = Coverslip, y = log(feature), color = Experiment)) + 
        geom_boxplot(outlier.size = .5) +
        geom_jitter(height = 0, alpha = 0.4, size = 0.5) +
      ...
      ggsave(paste(savePath, features[val], "_log.png", sep = ""), p, width = 10, height = 6)
    }
  }
}
...
```{r}
# annotation1 <- read_excel(paste(dirPath, "Analysis/DataForTrainingClassifier/MCAM-Myosin-Factin-ROCKi/AnnotationV2.xlsx", sep = ""))
# annotation4 <- read.csv(paste(dirPath, "Analysis/DataForTrainingClassifier/CopiedFromCopolarizationAndColocalizationOfERM/20211127_ROCKi_Annotation.csv", sep = ""))
# annotation4 <- annotation4 %>%
#   dplyr::mutate(Experiment = factor(Experiment, levels = unique(annotation4$Experiment)),
#          Replicate = factor(Replicate, levels = unique(annotation4$Replicate)),
...
#          Coverslip = factor(Coverslip, levels = unique(annotation4$Coverslip))) %>% # Preserves order of levels as input .csv
#   dplyr::mutate(ConditionMcam = str_remove(Condition, "_Stain.*"))
```

Matlab results are loaded here.
```{r}
...
#                                  "MCAM", "Factin", "Other")
# mcamFeatures <- read.csv(paste(dirPath, "Analysis/Paper/InputForGitHub/McamFeatures.csv", sep = "")) %>%
...
# 
# ## Scale features for each channel
...
# metricsScaled.MCAM <- rbind(metricsScaled1.V1.MCAM, metricsScaled4.V1.MCAM)
```
...
metricsScaled.MCAM <- read.csv(paste(dirPath, "Analysis/Paper/InputForGitHub/metricsScaledMCAM.csv", sep = ""))
...
Manually selected features chosen in the DevelopmentOfML_GitHub.Rmd file. 
```{r}
reducedFeatures.MCAM <- read.csv(paste(dirPath, "Analysis/Paper/InputForGitHub/SelectedFeaturesMcam.csv", sep = ""))[,"x"]
```

Train control function and initial training set created using Caret functions:
I am rerunning script on 7/9/22:
```{r}
...
metricsScaled.MCAM.Labeled <- merge(metricsScaled.MCAM, dplyr::select(manualComb, Object, ManualScoreMcam, ConfidenceMcam), by = "Object")
objectsMcamFactin <- metricsScaled.MCAM.Labeled %>% dplyr::select(Object)
set.seed(123)
objectsTrainMcam <- objectsMcamFactin[createDataPartition(metricsScaled.MCAM.Labeled$ManualScoreMcam[metricsScaled.MCAM.Labeled$Object %in% objectsMcamFactin$Object], p = 0.5, list = F, times = 1),]
```

Train classifiers using the upSample function:
```{r}
# mcamModels <- upSample(metricsScaled.MCAM.Labeled, "ManualScoreMcam", objectsTrainMcam, myControlUpsample, reducedFeatures.MCAM, timesUpsampled = 2)
```

Save classifiers:

Note: you MUST use prepareForSave function from extraTrees on the finalModel (S3: extraTrees object) to be able to use the classifier during a different session. Otherwise you will get the error "Error in .jcall(et$jobject, "[I", "getValues", toJavaMatrix2D(newdata)) : RcallMethod: attempt to call a method of a NULL object."
```{r}
# prepareForSave(mcamModels[["fittedModels"]][["extraTreesZero"]]$finalModel)
# prepareForSave(mcamModels[["fittedModels"]][["extraTreesOne"]]$finalModel)
# prepareForSave(mcamModels[["fittedModels"]][["extraTreesTwo"]]$finalModel)

# saveRDS(mcamModels, "E:/Maria Data/WRAMP/Paper/Figure_1_Analysis/MachineLearning/MachineLearningWithVersionControl/Thesis/DevelopmentOfUpsample/Classifiers/modelList_MCAM_20220709")
```

Read in saved models (to reload models without running training again):
```{r}
mcamModels <- readRDS(paste(dirPath, "Analysis/Paper/InputForGitHub/modelList_MCAM_20220709", sep = ""))
```

Generate classified data with two classifiers, where both models are required to predict a "OneEnd" classification.
```{r}
metricsCleanMcam.Classified <- classifyCleanMetricsCombinedModels(classifyCleanMetricsCombinedModels(metricsClean.V1, 
                                                               metricsScaled.MCAM, 
                                                                mcamModels[["fittedModels"]][["gbmTwo"]],
                                                mcamModels[["fittedModels"]][["extraTreesTwo"]], 
                                                columnName = "comboGbmExtraTwo_MCAM"),
                                                metricsScaled.MCAM, 
                                                                mcamModels[["fittedModels"]][["gbmOne"]],
                                                mcamModels[["fittedModels"]][["extraTreesOne"]], 
                                                columnName = "comboGbmExtraOne_MCAM") %>%
  ...
  unite("mainRep", c(mainCondition,repNum), remove = F)

```

Accuracy of individual models:
```{r}
analyzeResults(metricsScaled.MCAM.Labeled  %>%
                  dplyr::mutate(all = "All") %>% 
                 filter(!Object %in% mcamModels[["trainingSets"]][["Zero"]]), mcamModels[["fittedModels"]][["gbmZero"]], "ManualScoreMcam", "all")
nrow(metricsScaled.MCAM.Labeled  %>%
                  dplyr::mutate(all = "All") %>% 
                 filter(!Object %in% mcamModels[["trainingSets"]][["Zero"]]))

analyzeResults(metricsScaled.MCAM.Labeled  %>%
                  dplyr::mutate(all = "All") %>% 
                 filter(!Object %in% mcamModels[["trainingSets"]][["One"]]), mcamModels[["fittedModels"]][["gbmOne"]], "ManualScoreMcam", "all")

nrow(metricsScaled.MCAM.Labeled  %>%
                  dplyr::mutate(all = "All") %>% 
                 filter(!Object %in% mcamModels[["trainingSets"]][["One"]]))

analyzeResults(metricsScaled.MCAM.Labeled  %>%
                  dplyr::mutate(all = "All") %>% 
                 filter(!Object %in% mcamModels[["trainingSets"]][["Two"]]), mcamModels[["fittedModels"]][["gbmTwo"]], "ManualScoreMcam", "all")

nrow(metricsScaled.MCAM.Labeled  %>%
                  dplyr::mutate(all = "All") %>% 
                 filter(!Object %in% mcamModels[["trainingSets"]][["Two"]]))

analyzeResults(metricsScaled.MCAM.Labeled  %>%
                  dplyr::mutate(all = "All") %>% 
                 filter(!Object %in% mcamModels[["trainingSets"]][["Zero"]]), mcamModels[["fittedModels"]][["extraTreesZero"]], "ManualScoreMcam", "all")

analyzeResults(metricsScaled.MCAM.Labeled  %>%
                  dplyr::mutate(all = "All") %>% 
                 filter(!Object %in% mcamModels[["trainingSets"]][["One"]]), mcamModels[["fittedModels"]][["extraTreesOne"]], "ManualScoreMcam", "all")

analyzeResults(metricsScaled.MCAM.Labeled  %>%
                  dplyr::mutate(all = "All") %>% 
                 filter(!Object %in% mcamModels[["trainingSets"]][["Two"]]), mcamModels[["fittedModels"]][["extraTreesTwo"]], "ManualScoreMcam", "all")


```
```{r}
analyzeResults2(metricsCleanMcam.Classified %>%
                  merge(dplyr::select(manualComb, Object, ManualScoreMcam, ConfidenceMcam), by = "Object") %>%
                  filter(!Object %in% mcamModels[["trainingSets"]][["One"]]), 
                "comboGbmExtraOne_MCAM", "ManualScoreMcam", "all")

analyzeResults2(metricsCleanMcam.Classified %>%
                  merge(dplyr::select(manualComb, Object, ManualScoreMcam, ConfidenceMcam), by = "Object") %>%
                  filter(!Object %in% mcamModels[["trainingSets"]][["Two"]]), 
                "comboGbmExtraTwo_MCAM", "ManualScoreMcam", "all")
```
Save classified data:
```{r}
# write_csv(metricsCleanMcam.Classified, paste(dirPath, "Analysis/Paper/Output/UpsampleDevelopment_ClassifiedMetrics_MCAM.csv", sep = ""))
```

Variable importance:
```{r}
impExtraTrees <- varImp(mcamModels[["fittedModels"]][["extraTreesTwo"]], scale = T)
impGbm <- varImp(mcamModels[["fittedModels"]][["gbmTwo"]], scale = T)

# write.csv(impExtraTrees[["importance"]], paste(dirPath, "Analysis/Paper/Output/VariableImportance_extraTreesTwo.csv", sep = ""))
# write.csv(impGbm[["importance"]], paste(dirPath, "Analysis/Paper/Output/VariableImportance_gbmTwo.csv", sep = ""))

impExtraTreesSorted <- impExtraTrees[["importance"]] %>%
  arrange(desc(OneEnd))
impGbmSorted <- impGbm[["importance"]] %>%
  arrange(desc(Overall))

impBoth <- intersect(rownames(impExtraTreesSorted[1:20,]), rownames(impGbmSorted)[1:20])

impMetrics <- metricsScaled.MCAM %>%
  dplyr::select(all_of(impBoth[1:10]), Object) %>%
  merge(metricsCleanMcam.Classified %>% dplyr::select(Object, comboGbmExtraTwo_MCAM, mainRep), by = "Object") %>%
  gather(key = "Feature", value = "Value", -comboGbmExtraTwo_MCAM, -Object,-mainRep)

clr = c("OneEnd" = "#e66101", "BothEnds" = "#b2abd2", "None" = "#5e3c99")
for(val in 1:10){
  p <- ggplot(impMetrics %>% filter(Feature == impBoth[val]), aes(x = comboGbmExtraTwo_MCAM, y = Value, color = comboGbmExtraTwo_MCAM)) + 
...
#  ggsave(paste(dirPath, "Analysis/Paper/Output/",
#             impBoth[val], ".svg", sep = ""), plot = p, width = 12, height = 15, units = "cm", dpi = 600) 
}

# Sample size
impMetrics %>% 
  filter(Feature == impBoth[1]) %>%
  count(comboGbmExtraTwo_MCAM)
...
metricsCleanMcam.Classified %>% count(comboGbmExtraTwo_MCAM)
```

Plot features based on manual scoring:
```{r}
metricsPlotMan <- merge(metricsClean.V1, dplyr::select(manualComb, Object, ManualScoreMcam, ConfidenceMcam), by = "Object") # Removes small cells
...
metricsPlotMan %>% count(ManualScoreMcam)

# ggsave(paste(dirPath, "Analysis/Paper/Output/",
#              "medianDistCentr2Wramp_Manual", ".svg", sep = ""), width = 12, height = 15, units = "cm", dpi = 600)

# Plot with combined classes
ggplot(metricsPlotMan %>%
         dplyr::mutate(ManualScoreMcam = as.character(ManualScoreMcam)) %>%
         dplyr::mutate(twoClass = ifelse(ManualScoreMcam == "BothEnds", "None", ManualScoreMcam)) %>%
         dplyr::mutate(twoClass = factor(twoClass, levels = c("OneEnd", "None"))), aes(x = twoClass, y = medianDistCentr2Wramp_MCAM, color = twoClass)) + 
    geom_violin(draw_quantiles= c(0.25,0.5,0.75),
                width = 1.5,
                alpha = 1,
                scale = "count",
...
# ggsave(paste(dirPath, "Analysis/Paper/Output/",
#              "medianDistCentr2Wramp_Manual_TwoClasses", ".svg", sep = ""), width = 10, height = 13, units = "cm", dpi = 600)

metricsPlotMan %>%
         dplyr::mutate(ManualScoreMcam = as.character(ManualScoreMcam)) %>%
         dplyr::mutate(twoClass = ifelse(ManualScoreMcam == "BothEnds", "None", ManualScoreMcam)) %>%
         dplyr::mutate(twoClass = factor(twoClass, levels = c("OneEnd", "None"))) %>%
  count(twoClass)
```

Proportion of cells with Wrong Object (brightest object not at polarized region) & Bad Segmentation
```{r}
# Proportion of polarized cells where the brightest object in the cell did not correspond to the region visually identified as polarized
manualComb %>% count(ManualScoreMcam)
116/(116+940+2254)
```

Supplemental data:
```{r}
suppFeatures2 <- read.csv(paste(dirPath, "Analysis/Paper/InputForGitHub/McamFeatures.csv", sep = ""))

suppMetrics <- metricsCleanMcam.Classified %>%
  merge(manualComb %>% dplyr::select(ManualScoreMcam, Object, WrongObject), by="Object", all.x = T) %>%
  dplyr::select(all_of(suppFeatures2$suppFeatures2)) %>%
  dplyr::rename(Classification = comboGbmExtraTwo_MCAM)
colnames(suppMetrics)
suppMetrics %>% count(ManualScoreMcam)
metricsPlotMan %>% count(ManualScoreMcam)

identical(suppMetrics %>% dplyr::select(any_of(colnames(metricsCleanMcam.Classified))), metricsCleanMcam.Classified %>% dplyr::select(any_of(colnames(suppMetrics))) %>% dplyr::select(any_of(colnames(metricsCleanMcam.Classified))))

colnames(metricsScaled.MCAM)
identical(metricsScaled.MCAM$Object, suppMetrics$Object)

write_csv(suppMetrics, paste(dirPath, "Analysis/Paper/OutputForGitHub/SupplementalMcamFeatures.csv", sep = ""))
write_csv(metricsScaled.MCAM, paste(dirPath, "Analysis/Paper/OutputForGitHub/SupplementalMcamFeatures_Scaled.csv", sep = ""))
write.csv(mcamModels[["trainingSets"]][["Two"]], paste(dirPath, "Analysis/Paper/OutputForGitHub/FinalUpsampleTrainingSet.csv", sep = ""))
```
