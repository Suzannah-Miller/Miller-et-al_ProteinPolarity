...
NOTE #1: Code is shown following the training of each ML model where the sensitivity and specificity was measured using any manually labeled cells which were not part of the training set but were part of the labeled data used for iterative classifier training. These accuracy measurements (referred to as "preliminary accuracy") were used to evaluate and optimize the strategy for selecting initial training sets and to choose the model ultimately used to generate the classified data in the paper. HOWEVER, we realized later that these test sets were potentially biased because they were part of the iterative training process and not independent of the training set after the first iteration (a portion of misclassified cells were removed and substituted with a portion of the training set). So following the code for generating the "final classifications" is code for sampling a new test set of cells excluding any cells used during iterative classifier training. The images of these cells were blinded for manual classification and used to report the accuracy of the ML classification (Supplemental Tables 9 and 11). 

Following this is the code used to generate figures and the co-polarization/colocalization analysis.
...
NOTE #2: the variable "Object" in this script refers to a unique identifier assigned to every cell in the immunofluorescence data, which is the image name followed by an underscore and the number of the cell in the image based on the segmentation. This allows outputs from MATLAB, CellProfiler, and the ML classification to be compared or merged. This should not be confused with the image-analysis definition of object (a connected component in the image) which is used within the MATLAB scripts in reference to the brightest object (region) in cells. The author apologizes for what in retrospect was a poor choice in variable naming.

NOTE #3: the term "wramp" in the variable names for the features measured in MATLAB is synonymous with the "brightest object" in cells (the region of the cell with the highest integrated intensity). This is because the code was developed to identify cells with polarized proteins which are markers of WRAMP structures.
...
```{r}
dirPath <- "Z:/Suzannah_Thesis_Paper/ClonalCRISPR/GitHub/"
```
...
A subset of data was processed and classified during a preliminary analysis of images co-stained for MCAM and MSN. The code for selecting cells for manual classification and training the preliminary classifier are shown here commented out.
```{r}
# ## Preliminary classification based on classifier from ERM knockdown dataset
# prevModel <- readRDS("Z:/Suzannah_Thesis_Paper/Fig1Fig6Backup/Figure_6_Analysis/AllReplicatesAnalysisInR/AllReplicates_RevisedAnalysis_20221101/Output/ClassifiersForMcam/modelList20221104-NoTexture_85up")
# 
...
# 
# metricsClean.Classified.MCAM.prelim %>% group_by(Condition) %>% count(prelimClassMcam) %>% ungroup()
#
# ## Randomly sample objects to manually label for classifier training
# trainingObjects <- rbind(metricsClean.Classified.MCAM.prelim %>%
#   filter(str_detect(Condition, "Naive")) %>%
#   group_by(prelimClassMcam) %>%
...
# 
# write_csv(trainingObjects, paste(dirPath, "AnalysisInR/Output/trainingSet1_20221201.csv", sep = ""))
# trainingObjects <- read.csv(paste(dirPath, "AnalysisInR/Output/trainingSet1_20221201.csv", sep = ""))
# 
# ## Shuffle cells
# nCells <- length(trainingObjects$Object)
# set.seed(123)
# fileNameV1 <- paste(gsub(".nd2", "_Img_Cell", trainingObjects$Object)[sample(nCells, nCells, replace = F)], "_80thPctlNoClose.png", sep = "")
...
#    dplyr::mutate(pathNameImgV1 = paste("/Volumes/ahngrp/Suzannah_Thesis_Paper/CopolarizationAndColocalizationOfERM/Figure4_ROCKi_MSN-EZR-RDX-Actin/Analysis/MatlabAnalysis/WrampMetrics/Output/Figures/", imgNameV1, sep = ""),
...
#   dplyr::mutate(toPathNameImgV1 = paste(dirPath, "MatlabAnalysis/WrampMetrics/Output/Figures/TrainingSet1/", imgNameV1, sep = ""),
#           toPathNameImgV2 = paste(dirPath, "MatlabAnalysis/WrampMetrics/Output/Figures/TrainingSet1/", imgNameV2, sep = ""),
#           toPathNameFigV1 = paste(dirPath, "MatlabAnalysis/WrampMetrics/Output/Figures/TrainingSet1/", figNameV1, sep = ""),
#           toPathNameFigV2 = paste(dirPath, "MatlabAnalysis/WrampMetrics/Output/Figures/TrainingSet1/", figNameV2, sep = ""))
# # pathNameImgV1 is incorrect but this path was not used because these images/figures have the V2 name
# 
...
# 
# setwd(paste(dirPath, "MatlabAnalysis/WrampMetrics/Output/Figures/TrainingSet1/", sep = ""))
# write.csv(renameMatrix, "Blinding_TrainingSet1_20221201.csv")
...
#
# ## Training a preliminary ML classifier
# # set.seed(123)
# # balancedLabeling <- rbind(manualTrain1 %>%
# #                             filter(ManualScoreMcam == "BothEnds"),
# #                           manualTrain1 %>% 
# #                             filter(is.na(Segmentation), is.na(WrongObject), ManualScoreMcam != "BothEnds") %>%
# #                             group_by(ManualScoreMcam) %>%
# #                             slice_sample(n = 357, replace = F))
# write_csv(balancedLabeling, paste(dirPath, "AnalysisInR/Output/balancedLabeling_TrainingSet1.csv", sep = ""))
# balancedLabeling <- read.csv(paste(dirPath, "AnalysisInR/Output/balancedLabeling_TrainingSet1.csv", sep = ""))
# 
# # No duplicated scoring of the same cell
# sum(duplicated(balancedLabeling$Object))
# 
# metricsScaled.MCAM.labeled <- metricsScaled.MCAM %>%
#     merge(balancedLabeling %>% dplyr::select(ManualScoreMcam, ConfidenceMcam, Object), by = "Object") 
# 
# # Objects for starting training set:
# balancedLabeling.Control <- balancedLabeling %>% filter(str_detect(Condition, "Naive"))
# set.seed(123)
# trainingSetMcamObjects.Control <- balancedLabeling.Control$Object[createDataPartition(balancedLabeling.Control$ManualScoreMcam, p = 0.8, list = F, times = 1)]
# 
# # Control parameters for training from caret package:
...
#                                   classProbs = T)
# 
# mcamModel1_85.Control <- upSample85(metricsScaled.MCAM.labeled %>% filter(str_detect(Condition, "Naive")), "ManualScoreMcam", trainingSetMcamObjects.Control, myControlUpsample, reducedFeatures.MCAM, timesUpsampled = 2)
# prepareForSave(mcamModel1_85.Control[["fittedModels"]][["extraTreesZero"]]$finalModel)
# prepareForSave(mcamModel1_85.Control[["fittedModels"]][["extraTreesOne"]]$finalModel)
# prepareForSave(mcamModel1_85.Control[["fittedModels"]][["extraTreesTwo"]]$finalModel)
# saveRDS(mcamModel1_85.Control, paste(dirPath, "AnalysisInR/Output/ClassifiersForMcam/modelList20221205_85up_Control", sep = ""))
# 
# mcamModel2_85.Control <- upSample85(metricsScaled.MCAM.labeled %>% filter(str_detect(Condition, "Naive")), "ManualScoreMcam", trainingSetMcamObjects.Control, myControlUpsample, reducedFeatures.MCAM2, timesUpsampled = 2)
# prepareForSave(mcamModel2_85.Control[["fittedModels"]][["extraTreesZero"]]$finalModel)
# prepareForSave(mcamModel2_85.Control[["fittedModels"]][["extraTreesOne"]]$finalModel)
# prepareForSave(mcamModel2_85.Control[["fittedModels"]][["extraTreesTwo"]]$finalModel)
# saveRDS(mcamModel2_85.Control, paste(dirPath, "AnalysisInR/Output/ClassifiersForMcam/modelList20221205_85up_Control_NoTexture", sep = ""))
```

After prelminary analysis, more manual annotation was done here to create a larger/training and test set and to create labeled data for classification of the other ERM proteins.
...
Preliminary evaluation of accuracy of different combinations of classifiers.

NOTE: THE CODE SHOWN HERE USES THE TEST SETS USED DURING ITERATIVE CLASSIFIER TRAINING TO TEST THE CLASSIFIER FROM THE CORRESPONDING ROUND OF TRAINING. HOWEVER, AFTER THE INITIAL ROUND, THE NEW TEST SET IS NOT INDEPENDENT OF THE TRAINING SET (A PORTION OF THE MISCLASSIFIED CELLS HAVE BEEN REMOVED). THUS, AN INDEPENDENT TEST SET WAS LATER EVALUATED TO VALIDATE AND REPORT THE ACCURACY OF THE CLASSIFICATION MODEL (SHOWN LATER IN THIS NOTEBOOK). 
```{r}
...
# Two rounds of upsampling, 36 features
analyzeResults2(metricsClean.Classified.MCAM.Two.All %>% 
                  merge(manualTrain %>% filter(is.na(Segmentation)) %>% 
                          dplyr::select(ManualScoreMcam, ConfidenceMcam, Object), by = "Object") %>%
...
Preliminary accuracy of different combinations of classifiers.
...
# Two rounds of upsampling, balanced, 36 features
...
# One round of upsampling, 36 features 
...
# Two rounds upsampling, balanced data per cell line, 36 features
...
RDX polarization classifier (not used in paper):
```{r}
...
analyzeResults2(metricsClean.Classified.RDX.One.All.Balanced %>%
                  merge(manualTrainUpdated %>% filter(str_detect(Condition, "RDX"), is.na(Segmentation)) %>% dplyr::select(ManualScoreOther, ConfidenceOther, Object), by = "Object") %>%
                  filter(!Object %in% rdxModel2_85.All.Balanced[["trainingSets"]][["One"]]), "comboGbmExtraOne_RDX2", "ManualScoreOther", "mainCondition") 
...
FOR THE FINAL CLASSIFICATIONS, THE CLASSIFIER AFTER TWO ROUNDS OF ITERATIVE CLASSIFIER TRAINING BEGINNING WITH THE BALANCED TRAINING SET WAS USED FOR ALL PROTEINS.
...
After iterative classifier training, a separate test set of cells was sampled from the data (excluding any cells used during iterative training of the MCAM, MSN, EZR, and pERM classification models). This was used to report accuracy for the model (Supplemental Tables 9 and 11).

Sampled 40 cells per cell line per staining condition, except for the first replicate where 120 cells per cell line were sampled (there was only 1 staining condition).
```{r}
# Replicate 1
set.seed(135)
sample3a <- metricsClean.Classified.MCAM.Final %>%
  filter(!Object %in% manualTrain$Object) %>%
  filter(!Object %in% mcamModel2_85.All[["trainingSets"]][["Two"]], !Object %in% mcamModel2_85.All[["trainingSets"]][["One"]], !Object %in% mcamModel2_85.All[["trainingSets"]][["Zero"]]) %>% # Redundant
  filter(Experiment == "20220907_Pilot") %>%
  dplyr::mutate(mainRep = str_remove(Replicate, "_Stain.*")) %>%
  group_by(Replicate) %>%
...
# Replicate 2-5
set.seed(135)
sample3b <- metricsClean.Classified.MCAM.Final %>%
  filter(!Object %in% manualTrain$Object) %>%
  filter(!Object %in% mcamModel2_85.All[["trainingSets"]][["Two"]], !Object %in% mcamModel2_85.All[["trainingSets"]][["One"]], !Object %in% mcamModel2_85.All[["trainingSets"]][["Zero"]]) %>% # Redundant
  filter(!Experiment == "20220907_Pilot") %>%
  dplyr::mutate(mainRep = str_remove(Replicate, "_Stain.*")) %>%
...
sample3 <- rbind(sample3a, sample3b)
# # write_csv(sample3, "Z:/Suzannah_Thesis_Paper/ClonalCRISPR/Analysis20221211/CrisprAnalysisInR/Output/Test_Sample3_Seed135_20230901.csv")
```

Shuffle and blind cells from new test set for manual classification:
```{r}
# Check for duplicates
sum(duplicated(sample3$Object))
nrow(sample3 %>% filter(Object %in% manualTrain$Object))
nrow(sample3 %>% filter(Object %in% mcamModel2_85.All[["trainingSets"]][["Two"]]))
nrow(sample3 %>% filter(Object %in% mcamModel2_85.All[["trainingSets"]][["One"]]))
nrow(sample3 %>% filter(Object %in% mcamModel2_85.All[["trainingSets"]][["Zero"]]))


# # Shuffle cells
# nCells <- length(sample3$Object)
# set.seed(123)
# fileNameV1 <- paste(gsub(".nd2", "_Img_Cell", sample3$Object)[sample(nCells, nCells, replace = F)], "_80thPctlNoClose.png", sep = "")
# fileNameV2 <- gsub("_80thPctlNoClose.png", "_Img_Cell_80thPctlNoClose.png", gsub("_Img_Cell", "", fileNameV1))
# 
# renameMatrix <- data.frame(newNum = 1:(nCells), imgNameV1 = fileNameV1, imgNameV2 = fileNameV2) %>%
#   dplyr::mutate(newNameImg = paste("80thPctlNoClose_", as.character(newNum), "_Img.png", sep = ""),
...
#          figNameV1 = paste("Bg_", str_replace(imgNameV1, "Img", "Figure"), sep = ""),
#          figNameV2 = str_replace(imgNameV2, "Img", "Figure")) %>%
#    dplyr::mutate(pathNameImgV1 = paste("Z:/Suzannah_Thesis_Paper/ClonalCRISPR/Analysis20221211/MatlabAnalysis/WrampMetrics/Output/Figures/", imgNameV1, sep = ""),
#           pathNameImgV2 = paste("Z:/Suzannah_Thesis_Paper/ClonalCRISPR/Analysis20221211/MatlabAnalysis/WrampMetrics/Output/Figures/", imgNameV2, sep = ""),
#           pathNameFigV1 = paste("Z:/Suzannah_Thesis_Paper/ClonalCRISPR/Analysis20221211/MatlabAnalysis/WrampMetrics/Output/Figures/", figNameV1, sep = ""),
#           pathNameFigV2 = paste("Z:/Suzannah_Thesis_Paper/ClonalCRISPR/Analysis20221211/MatlabAnalysis/WrampMetrics/Output/Figures/", figNameV2, sep = "")) %>%
#   dplyr::mutate(toPathNameImgV1 = paste("Z:/Suzannah_Thesis_Paper/ClonalCRISPR/Analysis20221211/MatlabAnalysis/WrampMetrics/Output/Figures/ValidationSample3_20230901/", imgNameV1, sep = ""),
#           toPathNameImgV2 = paste("Z:/Suzannah_Thesis_Paper/ClonalCRISPR/Analysis20221211/MatlabAnalysis/WrampMetrics/Output/Figures/ValidationSample3_20230901/", imgNameV2, sep = ""),
#           toPathNameFigV1 = paste("Z:/Suzannah_Thesis_Paper/ClonalCRISPR/Analysis20221211/MatlabAnalysis/WrampMetrics/Output/Figures/ValidationSample3_20230901/", figNameV1, sep = ""),
#           toPathNameFigV2 = paste("Z:/Suzannah_Thesis_Paper/ClonalCRISPR/Analysis20221211/MatlabAnalysis/WrampMetrics/Output/Figures/ValidationSample3_20230901/", figNameV2, sep = ""))
# 
# 
# 
# file.copy(from = renameMatrix$pathNameImgV1, to = renameMatrix$toPathNameImgV1)
...
# file.copy(from = renameMatrix$pathNameFigV2, to = renameMatrix$toPathNameFigV2)
# 
# setwd(paste("Z:/Suzannah_Thesis_Paper/ClonalCRISPR/Analysis20221211/MatlabAnalysis/WrampMetrics/Output/Figures/ValidationSample3_20230901/", sep = ""))
# write.csv(renameMatrix, "Blinding_Sample3_20230901.csv")
# file.rename(renameMatrix$imgNameV1, renameMatrix$newNameImg)
# file.rename(renameMatrix$imgNameV2, renameMatrix$newNameImg)
# file.rename(renameMatrix$figNameV1, renameMatrix$newNameFig)
# file.rename(renameMatrix$figNameV2, renameMatrix$newNameFig)
...
Results of manual classification of new test set ("valid0901"). 

NOTE: During manual classification, manually assigned labels are rated as ambiguous or non-ambiguous, which is indicated in the "Confidence" columns in the manual scoring data. A confidence of 1 means a non-ambiguous "None" label, a confidence of 3 indicates a non-ambiguous "OneEnd" or "BothEnds" label. A confidence of 2 indicates an ambiguous classification from any class (OneEnd, None, or BothEnds). Thus, removing cells with a Confidence == 2, removes any cells with an ambiguous classification, and leaves cells assessed by the analyst as non-ambiguously OneEnd, None, or BothEnds.
```{r}
valid0901 <- read.csv(paste(dirPath, "Input/ValidationSample3_20230901_ManualScoring_Filled_Final.csv", sep = ""))
setdiff(valid0901$Object, sample3$Object)

valid0901 %>% count(Replicate)

# Double-check that no cells from classifier training are included
valid0901 %>% filter(Object %in% manualTrain$Object)
nrow(valid0901 %>% filter(!Object %in% Object %in% mcamModel2_85.All[["trainingSets"]][["Two"]],
                          !Object %in% Object %in% mcamModel2_85.All[["trainingSets"]][["One"]],
                          !Object %in% Object %in% mcamModel2_85.All[["trainingSets"]][["Zero"]],
                          !Object %in% Object %in% msnModel2_85.All.Balanced[["trainingSets"]][["Two"]],
                          !Object %in% Object %in% msnModel2_85.All.Balanced[["trainingSets"]][["One"]],
                          !Object %in% Object %in% msnModel2_85.All.Balanced[["trainingSets"]][["Zero"]],
                          !Object %in% Object %in% ezrModel2_85.All.Balanced[["trainingSets"]][["Two"]],
                          !Object %in% Object %in% ezrModel2_85.All.Balanced[["trainingSets"]][["One"]],
                          !Object %in% Object %in% ezrModel2_85.All.Balanced[["trainingSets"]][["Zero"]],
                          !Object %in% Object %in% permModel2_85.All.Balanced[["trainingSets"]][["Two"]],
                          !Object %in% Object %in% permModel2_85.All.Balanced[["trainingSets"]][["One"]],
                          !Object %in% Object %in% permModel2_85.All.Balanced[["trainingSets"]][["Zero"]])) ==
  nrow(valid0901)

# Cells which were in the validation set and manualTrain3 (which was only used in iterative training of the unused RDX classifier)
intersect(valid0901$Object, manualTrain$Object)
scoredTwice <- intersect(valid0901$Object, manualTrain3$Object) # 51 cells

test <- merge(valid0901, manualTrain3, by = "Object")
filter(test, ManualScoreMcam.x != ManualScoreMcam.y) %>% dplyr::select(Object, ManualScoreMcam.x, ConfidenceMcam.x, ManualScoreMcam.y, ConfidenceMcam.y) # New scores are all "OneEnd", suggesting scoring this time was very sensitive to polarization 

# Look at distribution of ambiguous and non-ambiguous manual labels
confValid0901 <- valid0901 %>%
  filter(is.na(Segmentation)) %>%
  dplyr::mutate(mainCondition = str_remove(Condition, "_Stain.*")) %>%
  group_by(ManualScoreMcam, Experiment, mainCondition) %>%
  count(ConfidenceMcam) %>%
  dplyr::mutate(perc = n/sum(n), nTot = sum(n)) %>%
  ungroup() %>%
  dplyr::mutate(ConfidenceMcam = as.character(ConfidenceMcam),
                mainCondition = factor(mainCondition, levels= c("Naive", "KKGK3", "KKGK4")))
...
Accuracy of MCAM classification:
```{r}
analyzeResults2(metricsClean.Classified.MCAM.Final %>% 
                  dplyr::mutate(mainCondition = str_remove(Condition, "_Stain.*")) %>%
                  merge(valid0901 %>% filter(is.na(Segmentation)) %>% 
                          dplyr::select(ManualScoreMcam, ConfidenceMcam, Object), by = "Object") %>%
                  filter(!Object %in% mcamModel2_85.All[["trainingSets"]][["Two"]]), # redundant
               "comboGbmExtraTwo_MCAM2",
               "ManualScoreMcam", "Experiment")
...
Accuracy of different iterations of MCAM training:
```{r}
classifyCleanMetricsCombinedModels <- function(metricsCleaned, metricsScaled, fit1, fit2, columnName){
  metricsCleaned <- arrange(metricsCleaned, Object)
  metricsScaled <- arrange(metricsScaled, Object)
...
metricsClean.Classified.MCAM.Zero <- classifyCleanMetricsCombinedModels(metricsClean.Classified.MCAM.Final, metricsScaled.MCAM, 
                                                                       mcamModel2_85.All[["fittedModels"]][["gbmZero"]],
                                                                       mcamModel2_85.All[["fittedModels"]][["extraTreesZero"]],
                                                                       "comboGbmExtraZero_MCAM2")

metricsClean.Classified.MCAM.One <- classifyCleanMetricsCombinedModels(metricsClean.Classified.MCAM.Final, metricsScaled.MCAM, 
                                                                       mcamModel2_85.All[["fittedModels"]][["gbmOne"]],
                                                                       mcamModel2_85.All[["fittedModels"]][["extraTreesOne"]],
                                                                       "comboGbmExtraOne_MCAM2")

analyzeResults2(metricsClean.Classified.MCAM.Zero %>%
                  merge(valid0901 %>% filter(is.na(Segmentation)) %>% 
                          dplyr::select(ManualScoreMcam, ConfidenceMcam, Object), by = "Object") %>%
                  filter(!Object %in% mcamModel2_85.All[["trainingSets"]][["Zero"]]) %>%
...
nrow(valid0901) == nrow(metricsClean.Classified.MCAM.Final %>%
                    merge(valid0901, by = "Object") %>%
                    filter(!Object %in% mcamModel2_85.All[["trainingSets"]][["Two"]],
                           !Object %in% mcamModel2_85.All[["trainingSets"]][["One"]],
                           !Object %in% mcamModel2_85.All[["trainingSets"]][["Zero"]])) 
```

Accuracy of MSN classifier:
```{r}
...
analyzeResults2(metricsClean.Classified.MSN.Final %>% 
                  ...
                  filter(!Object %in% msnModel2_85.All.Balanced[["trainingSets"]][["Two"]]),
               "comboGbmExtraTwo_MSN2",
               "ManualScoreOther", "Condition")

valid0901 %>%
  dplyr::mutate(mainCondition = str_remove(Condition, "_Stain.*"),
                 mainRep = str_remove(Replicate, "_Stain.*")) %>%
  filter(is.na(Segmentation),
         str_detect(Condition, "MSN")) %>%
  group_by(ConfidenceOther) %>%
  count(Condition) %>%
  ungroup() %>%
  group_by(Condition) %>%
  dplyr::mutate(nTot = sum(n)) %>%
...
valid0901 %>%
  filter(is.na(Segmentation), str_detect(Condition, "MSN")) %>%
  filter(ConfidenceOther != 2) %>%
  dplyr::mutate(wramp = ifelse(ManualScoreOther == "OneEnd", "Yes", "No")) %>%
  group_by(wramp) %>%
  count(Condition) %>%
  ungroup()
...
valid0901 %>%
  filter(is.na(Segmentation), str_detect(Condition, "EZR")) %>%
  filter(ConfidenceOther != 2) %>%
  dplyr::mutate(wramp = ifelse(ManualScoreOther == "OneEnd", "Yes", "No")) %>%
  group_by(wramp) %>%
  count(Condition) %>%
...
valid0901 %>%
  filter(is.na(Segmentation), str_detect(Condition, "pERM")) %>%
  filter(ConfidenceOther != 2) %>%
  dplyr::mutate(wramp = ifelse(ManualScoreOther == "OneEnd", "Yes", "No")) %>%
  group_by(wramp) %>%
  count(Condition) %>%
...
analyzeResults2(metricsClean.Classified.pERM.Final %>% 
                  ...
                  filter(!Object %in% permModel2_85.All.Balanced[["trainingSets"]][["Two"]]),
               "comboGbmExtraTwo_pERM2",
               "ManualScoreOther", "All")

metricsClean.Classified.pERM.Final %>% 
                  dplyr::mutate(mainCondition = str_remove(Condition, "_Stain.*"),
                                All = "All") %>%
                  merge(valid0901 %>% filter(is.na(Segmentation)), by = "Object") %>%
  group_by(ManualScoreOther) %>%
  count(comboGbmExtraTwo_pERM2) %>%
  ungroup()

metricsClean.Classified.pERM.Final %>% 
                  dplyr::mutate(mainCondition = str_remove(Condition, "_Stain.*"),
                                All = "All") %>%
                  merge(valid0901 %>% filter(is.na(Segmentation), ConfidenceOther != 2), by = "Object") %>%
  group_by(ManualScoreOther) %>%
  count(comboGbmExtraTwo_pERM2) %>%
  ungroup()
```
...
# Compile manual scoring from training and the independent test set
manualTrainAndTest <- rbind(manualTrain, valid0901,
                            manualTrain3 %>% filter(!Object %in% manualTrain$Object, !Object %in% valid0901$Object))
nrow(manualTrainAndTest) - (nrow(manualTrain) + nrow(valid0901) + nrow(manualTrain3)) # 51 cells scored in valid0901 and manualTrain3
sum(duplicated(manualTrainAndTest$Object))
...
supp <- suppTemp %>%
  ...
  merge(manualTrainAndTest %>% filter(is.na(Segmentation)) %>% dplyr::select(ManualScoreMcam, ConfidenceMcam, ManualScoreOther, ConfidenceOther, WrongObject, Object), all.x = T, all.y = T, by = "Object") %>%
  dplyr::mutate(ColocalizationPlot = ifelse(Object %in% colocMetricsNaive$Object, "+", ""),
                TestSet = ifelse(Object %in% valid0901$Object, "+", "")) %>%
...
manualTrainAndTest %>% group_by(ManualScoreMcam) %>% count(ManualScoreOther) %>% ungroup()

t1 <- filter(manualTrainAndTest, !is.na(ManualScoreMcam)) %>% arrange(Object)
t2 <- filter(supp, !is.na(ManualScoreMcam)) %>% arrange(Object)
setdiff(t1$Object, t2$Object)
nrow(manualTrainAndTest %>% filter(is.na(Segmentation)))
identical(t1$ManualScoreMcam, t2$ManualScoreMcam)
identical(t1$ManualScoreOther, t2$ManualScoreOther)
...
write_csv(supp[1:10000,], paste(dirPath, "Output/SupplementalData_NaiveCrispr_Resubmission_Part1.csv", sep = "")) # Whole supplemental file too large to upload to GitHub
write_csv(supp[10001:nrow(supp),], paste(dirPath, "Output/SupplementalData_NaiveCrispr_Resubmission_Part2.csv", sep = ""))
```
...
Percentage of cells with "OneEnd" classification where brightest object is not at the polarized region:
```{r}
# Not including validation set (new test set) scoring because of inconsistencies in scoring of ambiguous cells
manualTrainUpdated %>%
  filter(is.na(Segmentation), !Object %in% valid0901$Object) %>%
...
Percentage of cells with "BothEnds"
```{r}
manualTrainAndTest %>%
    filter(is.na(Segmentation)) %>%
